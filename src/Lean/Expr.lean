...
/--
Structural equality check for expressions that treats universe levels as equal
if they are equivalent after normalization (e.g., `max u v` vs `max v u`).
-/
partial def eqvNormLevels (a b : Expr) : Bool :=
